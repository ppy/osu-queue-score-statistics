...
namespace osu.Server.Queues.ScoreStatisticsProcessor.Commands.Maintenance
{
    ...
    public class VerifyImportedScoresCommand
    {
        ...
        public async Task<int> OnExecuteAsync(CancellationToken cancellationToken)
        {
            ...
            Console.WriteLine();
            Console.WriteLine($"Verifying from scores starting from {lastId}");
                ...
                while (!cancellationToken.IsCancellationRequested)
                {
                    ...
                    using (var transaction = await conn.BeginTransactionAsync(cancellationToken))
                    {
                        ...
                        foreach (var score in highScores)
                        {
                            ...
                            dynamic? performance = await conn.QuerySingleOrDefaultAsync("SELECT pp FROM score_performance WHERE score_id = @id", score, transaction);
